...
def fgsm_attack(image: torch.Tensor, data_grad: torch.Tensor, epsilon = 0.25):
...
def pgd_attack(model, data: torch.Tensor, target, criterion, args):
    alpha = args[ALPHA]
    ...
    for _ in range(num_iter):
        ...
        with torch.no_grad():
            x = fgsm_attack(x, x.grad, epsilon)
    
        delta = x - data
        norm = torch.linalg.norm(delta).item()
        clipped_norm = min(norm, alpha)
        
        # normalize, then scale by clipped norm
        x = data + delta / norm * clipped_norm
...
def test_attack(model, test_loader, attack_function, attack_args):
    ...
    for data, target in test_loader:
        ...
        if attack_function == FGSM: 
            ...
            loss.backward()
            perturbed_data = fgsm_attack(data, data.grad) # TODO: add epsilon from attack args?
